...
import { LogParserResponse_4dfe1dd_LogEntry, LogParserResponse_4dfe1dd_LogFile } from "../openapi";
...
class ConcreteLogFilteringService implements ILogFilteringService {
    ...
    get filteredEntries() {
        return this.registeredFilters.reduce((e, filter) => filter.filter(e), this.logFile?.log_entries || []);
    }
...
const LogFilteringServiceContext = createContext<ILogFilteringService | undefined>(undefined);

const LogFilteringService = (props: PropsWithChildren) => {
    return <LogFilteringServiceContext.Provider value={new ConcreteLogFilteringService()}>
        {props.children}
...
const FilterUiObserver = observer(({filteringService}: ObserverProps) => filteringService.filtersUi)
