...
const SeqTable = observer(({ filter, firstLast }: { filter: EventSequenceFilteringStrategy, firstLast: number }) => (
  <>
  <InputText type="text" placeholder="Code"
    value={filter.getInserting(firstLast).code}
    onChange={ (e) => (filter.getInserting(firstLast).code = e.target.value)} />
  <InputText type="text" placeholder="Value"
    value={filter.getInserting(firstLast).value}
    onChange={ (e) => (filter.getInserting(firstLast).value = e.target.value)} />
  <Button type="button"
    onClick={(e) => filter.addItem(firstLast)}
  >Add</Button>

  <DataTable
    value={firstLast === filter.FIRST ? filter.firstValues : filter.lastValues}
    editMode="row"
    dataKey="id"
    onRowEditComplete={ (e) => filter.editItem(e, firstLast) }
    onRowReorder={ (e) => filter.reorderItems(e, firstLast) } reorderableRows
    >
      ...
      <Column body={(rowData, props) => 
        <div className="flex flex-wrap gap-2">
          <Button type="button" icon="pi pi-trash" severity="danger" onClick={(e) => filter.deleteItem(props.rowIndex, firstLast)}></Button>
        </div>
...
const SeqCard = observer(({ filter }: { filter: EventSequenceFilteringStrategy }) => (
  <>
    <SeqTable filter={filter} firstLast={filter.LAST} />
    <SeqTable filter={filter} firstLast={filter.FIRST}  />
    <InputNumber value={filter.time} onValueChange={(e) => (filter.time = e.target.value ?? 0)} />
  </>
));
