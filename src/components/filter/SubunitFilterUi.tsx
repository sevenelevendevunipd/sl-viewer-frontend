...
const SubunitFilterObserverUi = observer(({ filter }: ObserverProps) => {
  ...
  useEffect(() => {
    setExpandedKeys(
      Object.fromEntries(filter.subunitTree.map((entry) => [entry.key, true]))
    );
  }, [filter.subunitTree]);
  return (
    <Tree
      value={filter.subunitTree}
      selectionMode="checkbox"
      selectionKeys={filter.selectedSubunits}
      onSelectionChange={(e) =>
        (filter.selectedSubunits = e.value as TreeCheckboxSelectionKeys)
      }
