...
function selectTreeEntry(entry: TreeNode, selectedEntries: TreeCheckboxSelectionKeys): void {
...
export class SubunitFilteringStrategy implements LogFilteringStrategy {
    ...
    constructor(logFile: LogFile) {
        ...
        this.subunitTree = Object.entries(logFile.units_subunits).map(([unit, value]) => {
            return {
                key: unit,
    ...
    get filterSet() {
        return new Set(Object.keys(this.selectedSubunits).filter(k => k.startsWith('s')).map(k => parseInt(k.substring(1))));
    }
